-- Incremental Staging Transformation for gbq_shopify_transaction
-- Source: mck_src.gbq_shopify_transaction_histunion
-- Target: mck_stg.gbq_shopify_transaction
-- Mode: Incremental with Deduplication
-- Partition: id | Order: incremental_date desc, created_at desc
...
WITH cleaned_data AS (
  ...
  WHERE time > (
    ...
    WHERE table_name = 'gbq_shopify_transaction_histunion' AND project_name = 'staging'
  )
),
final_data AS (
  SELECT
